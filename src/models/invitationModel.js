...
const INVITATION_COLLECTION_SCHEMA = Joi.object({
  ...
  // Lời mời là board thì sẽ sẽ lưu thêm dữ liệu boardInvitation  - optional
...
const createNewBoardInvitation = async (data) => {
  try {
    ...
    //Biến đổi một số dữ liệu liên quan tới objectId chuẩn 
...
const findOneById = async (invitationId) => {
  try { const result = await GET_DB().collection(INVITATION_COLLECTION_NAME).findOne({ _id: new ObjectId(invitationId) })
    return result
  } catch (error) {
    throw error
  }
