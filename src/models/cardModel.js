...
const unshiftNewComment = async (cardId, commentData) => {
  try { const result = await GET_DB().collection(CARD_COLLECTION_NAME).findOneAndUpdate(
    { _id: new ObjectId(cardId) },
...
const updateMembers = async (cardId, incomingMemberInfo) => {
  try {
    ...
    if (incomingMemberInfo.action === CARD_MEMBER_ACTION.ADD) {
      updateCondition = { $addToSet: { memberIds: new ObjectId(incomingMemberInfo.userId) } }
    }
